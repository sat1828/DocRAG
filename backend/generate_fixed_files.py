"""
Generate all fixed Python files with correct syntax.
Run: python generate_fixed_files.py
"""
import os
...
class GSTTools:
    ...
    def flag_legal_risks(text: str) -> List[Dict[str, Any]]:
        ...
        if re.search(r'force\s*majeure|act\s*of\s*god|unforeseen\s*events', text, re.IGNORECASE):
            risks.append({"category": "Force Majeure", "severity": "medium", "description": "Force Majeure clause detected", "section": "Extract from context"})
        if re.search(r'penalty|liquidated\s*damages|compensation\s*for\s*breach', text, re.IGNORECASE):
            risks.append({"category": "Penalty Clause", "severity": "high", "description": "Penalty clause found", "section": "Extract from context"})
        if re.search(r'termination|terminate\s*agreement|notice\s*period', text, re.IGNORECASE):
            risks.append({"category": "Termination", "severity": "medium", "description": "Termination clause detected", "section": "Extract from context"})
        if re.search(r'jurisdiction|arbitration|dispute\s*resolution', text, re.IGNORECASE):
            risks.append({"category": "Dispute Resolution", "severity": "medium", "description": "Jurisdiction clause found", "section": "Extract from context"})
        if re.search(r'gst|goods\s*and\s*services\s*tax', text, re.IGNORECASE):
            if not GSTTools.extract_gstins(text):
                risks.append({"category": "GST Compliance", "severity": "high", "description": "GST mentioned but no valid GSTIN found", "section": "N/A"})
        if re.search(r'indemnif|hold\s*harmless', text, re.IGNORECASE):
            risks.append({"category": "Indemnity", "severity": "high", "description": "Indemnity clause detected", "section": "Extract from context"})
...
class ChromaService:
    ...
    def query(self, collection_name: str, query_embedding: List[float], filter: Dict[str, Any], n_results: int = 10) -> Dict[str, Any]:
        try:
            collection = self.get_or_create_collection(collection_name)
...
# 6. rag_agent.py (fixed with proper LangGraph)
rag_agent_py = r'''"""
...
class RAGAgent:
    def __init__(self):
        ...
        self.system_prompt = """You are an expert Indian tax and legal assistant.
Rules:
1. Answer ONLY using provided context
2. Cite specific page numbers
3. If context doesn't contain answer, say "I cannot confirm this"
4. For GST calculations, use gst_calculator tool
5. Flag compliance risks
6. Never hallucinate"""
    ...
    def _retrieve(self, state: AgentState) -> Dict[str, Any]:
        ...
        for i, chunk in enumerate(chunks[:5]):
            ...
            context_parts.append(f"[Source {i+1} - Page {page} - {modality}]\n{chunk['content']}")
        return {"retrieved_chunks": chunks[:5], "context": "\n\n".join(context_parts)}

    def _generate(self, state: AgentState) -> Dict[str, Any]:
        messages = [SystemMessage(content=self.system_prompt), HumanMessage(content=f"Context:\n{state['context']}\n\nQuestion: {state['query']}")]
    ...
    def _verify(self, state: AgentState) -> Dict[str, Any]:
        ...
        grounded_count = sum(1 for s in answer_sentences if sum(1 for w in s.lower().split() if w in context.lower()) > len(s.lower().split()) * 0.3)
...
files = {
    ...
    os.path.join(base, "services", "gst_tools.py"): gst_tools_py,
    os.path.join(base, "services", "chroma_client.py"): chroma_client_py,
    os.path.join(base, "services", "embeddings.py"): embeddings_py,
    os.path.join(base, "services", "rag_agent.py"): rag_agent_py,
}
...
demo_path = os.path.join(base, "..", "demo_server.py")
if os.path.exists(demo_path):
    os.remove(demo_path)
    print(f"\nDeleted: demo_server.py")
